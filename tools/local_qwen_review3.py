# -*- coding: utf-8 -*-
"""本地 Qwen3-TTS 第三版：参考句按角色定制 + 输出裁剪开头毛刺 + 仅日语。"""
import io
import json
import os
import re
...
OUT_DIR = TOOLS / "review_audio_local3"
...
def pick_reference(char_id):
    ...
    for name in os.listdir(role_dir):
        ...
        if char_id in CUSTOM_LONG:
            # 陈述句、较长（100-220KB），排除疑问/感叹/语气词结尾
...
def main():
    ...
    try:
        for role, char_id in ROLE_MAP.items():
            ...
            print("ref %s: %s (%dB)" % (char_id, ref_text[:60], os.path.getsize(src_mp3)), flush=True)
                ...
                try:
                    ...
                    subprocess.run(
                        [FFMPEG, "-y", "-v", "error", "-i", str(out_raw),
                         "-af", "silenceremove=start_periods=1:start_threshold=-40dB:start_silence=0.03",
