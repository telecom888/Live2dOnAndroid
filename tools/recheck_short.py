...
FILES = [
    ...
    ("rana", 17), ("rana", 33), ("rana", 35), ("rana", 36), ("rana", 37), ("soyo", 35),
]
...
if __name__ == "__main__":
    ...
    with cf.ThreadPoolExecutor(max_workers=6) as ex:
        futs = {ex.submit(one, c, i): (c, i) for c, i in FILES}
    ...
    for c, i, text in out:
        exp = tips[c][i]["text"]
        print(f"== {c}/{i} ==  dur={round((VOICES/c/'ja'/f'{i}.mp3').stat().st_size/16000,2)}KB")
        print(f"  期望: {exp}")
        print(f"  转写: {text}")
